...
\subsection{提交记录}
本仓库按题、按实例增量提交（非单次全量提交）。本周（第三周）共提交 \textbf{17} 次 commit，
覆盖第 9～12 题、实例 1～11 及解题感悟，每次提交只针对一个逻辑单元，保留清晰的提交历史。
...
【本节写你上课时给助教检查的内容和结果：例如课堂演示、小测、口头回答、
  助教记录的检查情况等。这部分必须是你自己的真实课堂记录，请自行补充。】

% ============================================================
\section{课后练习（>10 个实例）}

本周共完成 \textbf{4 道题目}（第 9～12 题）与 \textbf{11 个实例}（实例 1～11），
合计 15 项，满足「每次课完成 >10 个实例」的要求。下面每项先放题目原文，再放运行结果截图。
