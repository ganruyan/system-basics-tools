...
\textbf{结果：}
\shot{第 13 题（a）ruff 与 pytest 检查结果}{report_imgs/q13a.png}{0.72\textwidth}
\shot{第 13 题（b）check.sh 运行结果}{report_imgs/q13b.png}{0.72\textwidth}
...
\textbf{结果：}
\shot{第 14 题（a）Makefile 依赖与首次构建}{report_imgs/q14a.png}{0.62\textwidth}
\shot{第 14 题（b）增量构建与 clean 清理}{report_imgs/q14b.png}{0.62\textwidth}
...
\textbf{结果：}
\shot{第 15 题（a）本地服务与 curl 获取数据}{report_imgs/q15a.png}{0.72\textwidth}
\shot{第 15 题（b）jq 筛选与生成的 summary.md}{report_imgs/q15b.png}{0.72\textwidth}
...
\textbf{结果：}
\shot{第 16 题（a）make check 与 make build}{report_imgs/q16a.png}{0.72\textwidth}
\shot{第 16 题（b）wheel 的 SHA-256 与最终提交}{report_imgs/q16b.png}{0.72\textwidth}
...
\shot{实例 1：grep 与 semgrep 匹配结果}{report_imgs/n01a.png}{0.85\textwidth}
...
\shot{实例 2：正则提取与修复后的结果}{report_imgs/n02a.png}{0.85\textwidth}
...
\shot{实例 3（a）pre-commit 拦截与报错}{report_imgs/n03a.png}{0.55\textwidth}
\shot{实例 3（b）修复后提交通过}{report_imgs/n03b.png}{0.55\textwidth}
...
\shot{实例 4（a）初始覆盖率 62\%}{report_imgs/n04a.png}{0.72\textwidth}
\shot{实例 4（b）补充测试后覆盖率 100\%}{report_imgs/n04c.png}{0.72\textwidth}
...
\centering\includegraphics[width=\textwidth]{report_imgs/n05a.png}
\caption{实例 5（a）注入错误后门禁拦截推送}
\end{minipage}\hfill
\begin{minipage}{0.46\textwidth}
\centering\includegraphics[width=\textwidth]{report_imgs/n05b.png}
\caption{实例 5（b）修复后三级门禁通过}
\end{minipage}
\end{figure}
...
\shot{实例 6（a）替换前 grep 查看多种用途}{report_imgs/n06a.png}{0.72\textwidth}
\shot{实例 6（b）替换后验证普通横线未被改动}{report_imgs/n06b.png}{0.72\textwidth}
...
\shot{实例 7（a）make 构建生成产物}{report_imgs/n07a.png}{0.72\textwidth}
\shot{实例 7（b）make clean 清理产物}{report_imgs/n07b.png}{0.72\textwidth}
...
\shot{实例 8：Cargo.toml 五种版本约束及场景}{report_imgs/n08a.png}{0.85\textwidth}
...
\shot{实例 9（a）构建失败拒绝提交}{report_imgs/n09a.png}{0.62\textwidth}
\shot{实例 9（b）修复后提交成功}{report_imgs/n09b.png}{0.62\textwidth}
...
\shot{实例 10（a）shellcheck 报错}{report_imgs/n10a.png}{0.72\textwidth}
\shot{实例 10（b）修复后批量检查通过}{report_imgs/n10b.png}{0.72\textwidth}
...
\shot{实例 11（a）proselint 报错}{report_imgs/n11a.png}{0.72\textwidth}
\shot{实例 11（b）干净文本检查通过}{report_imgs/n11b.png}{0.72\textwidth}

% ================= 4. 解题感悟 =================
\section{解题感悟}

这一周，我的第一个坑还是老问题——环境。装 pre-commit 时 Ubuntu 直接报 \texttt{externally-managed-environment}，全局 pip 装不了，后来才想到用虚拟环境解决。虽然这类问题看报错就能定位，但前几次还是会懵。总结一下这次实验遇到的困难，首先 proselint 新版本命令格式变成了 \texttt{proselint check}，直接按旧教程敲文件名会报错；其次 Makefile 里命令前面必须是 Tab 而不是空格；还有，git 钩子写好后要 \texttt{chmod +x} 才会执行……最后，这周做 AI 辅助修复的练习时我也意识到，工具给的代码只能当草稿，每一步改动还是得自己检查，确认没有问题才能用。
