...
本次课程的\textbf{报告、练习、代码}均已上传至个人 GitHub 仓库，采用\textbf{增量提交}（分多次 commit）：

\begin{center}
\url{https://github.com/ganruyan/system-basics-tools}
\end{center}

本周（第 2 周）的提交按内容拆分为 10 次独立 commit：

\begin{lstlisting}[language=bash]
a0ea30e 添加 .gitignore
89b5128 第二周实验报告
3b83f8f 第五题 信号与任务控制
07946b1 第六题 语义重构
85371e8 第七题 归并排序调试
...
57cdce2 修正第七题测试
f761316 补充截图和解题感悟
7fa9247 补充实例 1、7、9 截图
\end{lstlisting}

GitHub 上的 commit 记录截图如下：
...
\textbf{说明：} 脚本顶部的 \cmd{trap '...' TERM INT} 捕获终止信号，先写入 \cmd{cleanup.log} 再 \cmd{exit 0}，实现优雅退出。\cmd{jobs -p} 直接返回任务 PID。补充练习中还通过 \cmd{sleep 10000} 配合 \cmd{pgrep}/\cmd{pkill} 验证了按名称查找并终止后台任务：
...
\begin{enumerate}[itemsep=2pt, topsep=3pt]
  \item 最初我为了省事，把整周内容一次性全部提交，导致提交记录没有什么信息量。后面按一个题一次提交后，每次 commit 都对应一个明确的内容，回看时能立刻定位。尤其是修复 q07 测试期望值笔误时。
  \item 调试器比 print 更精准。第 7 题如果只靠 print 会反复的猜测错误；用调试器设断点之后，一步就锁定了缺陷。这让我体会到调试比打印看结果高效得多。
  \item 在第 8 个实例中我先用 cProfile -s cumulative 定位到 list 线性查找，再换成 set，整体从 $O(n^2)$ 降到 $O(n)$，且 count 结果保持不变。所以还是得测量之后再去优化。
  \item 命令行工具极大的提升了我的排查效率，比如用 netstat + taskkill 定位端口、strace 观察系统调用、hyperfine 做基准、htop 看负载……
\end{enumerate}
